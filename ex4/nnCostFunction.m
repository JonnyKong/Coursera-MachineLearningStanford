...
h = sigmoid(z3);
y_helper = zeros(size(h));
for i = 1 : m
    y_helper(i, y(i)) = 1;
end
J = (-1 / m) * sum(sum(y_helper .* log(h) + (1 - y_helper) .* log(1 - h)));
...
accu2 = zeros(size(Theta2));
accu1 = zeros(size(Theta1));
...
for i = 1 : m
    ...
    z3 = Theta2 * [1; a2];
    a3 = sigmoid(z3);
    sigma3 = a3 - (y_helper(i, :))';
    accu2 = accu2 + sigma3 * [1; a2]';
    %sigma2 = Theta2' * sigma3 .* sigmoidGradient([1;z2]);
    %accu2 = accu2 + sigma3 * [1; a2]';
    %accu1 = accu1 + sigma2(2 : end) * [1; a1]';
    sigma2 = (Theta2(:, 2:end))' * sigma3 .* sigmoidGradient(z2);
    accu1 = accu1 + sigma2 * [1; a1]';
end
